...
import {Activity,Anchor,BarChart3,Building2,Calculator,ChevronLeft,ChevronRight,FileCheck2,FileText,History,LayoutDashboard,Menu,Palette,Printer,Settings2,ShieldCheck,Ship,Truck,Wallet,X,LogOut,Sun,Moon,BookOpen} from 'lucide-react';
...
const groups=['عملیات','حمل و اسناد','ترخیص','مالی و خروج','کنترل'];
...
export const AppShell:React.FC<{children:React.ReactNode}>=({children})=>{
 const location=useLocation();const navigate=useNavigate();const{profile,signOut}=useAuth();const{theme,setTheme,comfort,setComfort,density,setDensity,sidebarCollapsed,setSidebarCollapsed}=useAppearance();
 const[mobileOpen,setMobileOpen]=useState(false);const[appearanceOpen,setAppearanceOpen]=useState(false);
 const bare=location.pathname==='/login'||location.pathname==='/onboarding';
 if(bare)return <>{children}</>;
 const isActive=(to:string)=>to.includes('?')?location.pathname===to.split('?')[0]&&new URLSearchParams(location.search).get('tab')===new URLSearchParams(to.split('?')[1]).get('tab'):location.pathname===to;
 return <div className="app-shell min-h-screen" dir="rtl">
  <aside className={`app-sidebar hidden md:flex ${sidebarCollapsed?'is-collapsed':''}`}>
   <div className="app-brand"><div className="app-brand-mark"><Ship size={22}/></div>{!sidebarCollapsed&&<div className="min-w-0"><div className="font-black tracking-tight">Customs OS</div><div className="text-[10px] app-muted truncate">Customs & Logistics</div></div>}</div>
  ...
  <main className={`app-main ${sidebarCollapsed?'sidebar-compact':''}`}><header className="app-topbar"><div className="flex items-center gap-3 min-w-0"><div className="hidden lg:block"><div className="text-[11px] app-muted">سامانه مدیریت عملیات گمرکی و لجستیک</div><div className="font-bold truncate">{nav.find(x=>isActive(x.to))?.label||'Customs OS'}</div></div><div className="flex-1"/><div className="hidden sm:flex items-center gap-2"><span className="app-user-chip"><span className="h-7 w-7 rounded-full grid place-items-center bg-[var(--primary)] text-white text-xs font-black">{(profile?.full_name||'C').slice(0,1)}</span><span className="text-xs"><b>{profile?.full_name||'کاربر'}</b><span className="block app-muted">{roles[profile?.role||'']||''}</span></span></span><button className="icon-btn" title="ظاهر" onClick={()=>setAppearanceOpen(v=>!v)}><Palette size={18}/></button><button className="icon-btn" title="خروج" onClick={()=>signOut()}><LogOut size={18}/></button></div></div></header>
   {appearanceOpen&&<div className="appearance-panel"><div className="flex items-center justify-between mb-3"><div><b>ظاهر و مطالعه</b><div className="text-[11px] app-muted">تنظیمات برای استفاده طولانی‌مدت</div></div><Settings2 size={18}/></div><div className="text-xs font-semibold mb-2">تم</div><div className="segmented">{(['system','light','dark'] as const).map(v=><button key={v} onClick={()=>setTheme(v)} className={theme===v?'selected':''}>{v==='system'?'سیستم':v==='light'?'روشن':'تیره'}{v==='light'?<Sun size={14}/>:v==='dark'?<Moon size={14}/>:<Palette size={14}/>}</button>)}</div><div className="text-xs font-semibold mt-4 mb-2">حالت مطالعه</div><div className="segmented">{(['normal','soft','warm','reading'] as const).map(v=><button key={v} onClick={()=>setComfort(v)} className={comfort===v?'selected':''}>{v==='normal'?'عادی':v==='soft'?'ملایم':v==='warm'?'گرم':'کتاب‌خوان'}{v==='reading'&&<BookOpen size={14}/>}</button>)}</div><div className="text-xs font-semibold mt-4 mb-2">تراکم رابط</div><div className="segmented">{(['comfortable','compact'] as const).map(v=><button key={v} onClick={()=>setDensity(v)} className={density===v?'selected':''}>{v==='comfortable'?'راحت':'فشرده'}</button>)}</div></div>}
