...
import {Activity,Anchor,BarChart3,Building2,Calculator,ChevronLeft,ChevronRight,FileCheck2,FileText,FileSearch,History,LayoutDashboard,Menu,Palette,Printer,Settings2,ShieldCheck,Ship,Truck,X,LogOut,Sun,Moon,Wallet,ClipboardList,FolderOpen} from 'lucide-react';
...
const nav:NavItem[]=[
 ...
 {label:'کشتیرانی',en:'Maritime',to:'/maritime',icon:Anchor,group:'حمل و اسناد',roles:management},
 ...
 {label:'فایل‌منیجر اسناد محموله',en:'Shipment Document Manager',to:'/documents/extract',icon:FolderOpen,group:'حمل و اسناد',roles:['owner','admin','broker','warehouse','client']},
 {label:'۱- ورود اطلاعات قبل اظهار',en:'Pre-Declaration Data',to:'/operations?tab=pre-declaration',icon:ClipboardList,group:'گمرکی',roles:['owner','admin','broker','client']},
 {label:'۲- اظهار / EPL',en:'Declaration / EPL',to:'/operations?tab=declaration',icon:FileCheck2,group:'گمرکی',roles:management},
 {label:'۳- صدور پروانه / درب خروج',en:'Permit / Cargo Exit',to:'/exit',icon:Truck,group:'گمرکی',roles:['owner','admin','broker','warehouse']},
...
export const AppShell:React.FC<{children:React.ReactNode}>=({children})=>{
 const location=useLocation();const navigate=useNavigate();const{profile,signOut}=useAuth();const{theme,setTheme,density,setDensity,sidebarCollapsed,setSidebarCollapsed}=useAppearance();const[mobileOpen,setMobileOpen]=useState(false),[appearanceOpen,setAppearanceOpen]=useState(false);const bare=location.pathname==='/login'||location.pathname==='/onboarding';
 ...
 <aside className={`app-sidebar hidden md:flex ${sidebarCollapsed?'is-collapsed':''}`}><div className="app-brand"><div className="app-brand-mark"><Ship size={22}/></div>{!sidebarCollapsed&&<div className="min-w-0"><div className="font-black tracking-tight">Customs OS</div><div className="text-[10px] app-muted truncate">Customs & Logistics</div></div>}</div>
 ...
 <div className="app-content">{children}</div></main><OfflineQueueStatus/><AIWorkspace pageContext={location.pathname+location.search}/></div>;
};
