...
import {CustomsLogo} from './CustomsLogo';
import {AIWorkspace} from './AIWorkspace';
type NavItem={label:string;en:string;to:string;icon:React.ElementType;group:string;roles?:UserRole[]};
const allStaff:UserRole[]=['owner','admin','broker','accountant','warehouse'];const management:UserRole[]=['owner','admin','broker'];const finance:UserRole[]=['owner','admin','accountant'];
const nav:NavItem[]=[
 ...
 {label:'فایل‌منیجر اسناد محموله',en:'Shipment Documents',to:'/documents',icon:FolderOpen,group:'حمل و اسناد',roles:['owner','admin','broker','warehouse','client']},
 ...
 {label:'۳- پروانه / درب خروج',en:'Permit / Exit',to:'/operations?tab=exit',icon:Truck,group:'گمرکی',roles:['owner','admin','broker','warehouse']},
 {label:'ارزش‌گذاری',en:'Valuation',to:'/operations?tab=pre-declaration',icon:Calculator,group:'ترخیص',roles:management},
 {label:'اسناد و مجوزها',en:'Documents & Permits',to:'/documents',icon:FileText,group:'ترخیص',roles:['owner','admin','broker','warehouse','client']},
 ...
 {label:'خروج کالا',en:'Cargo Exit',to:'/operations?tab=exit',icon:Truck,group:'مالی و خروج',roles:['owner','admin','broker','warehouse']},
...
export const AppShell:React.FC<{children:React.ReactNode}>=({children})=>{
 const location=useLocation(),navigate=useNavigate(),{profile,signOut}=useAuth();const{theme,setTheme,comfort,setComfort,sidebarCollapsed,setSidebarCollapsed}=useAppearance();const[mobileOpen,setMobileOpen]=useState(false);const bare=location.pathname==='/login'||location.pathname==='/onboarding';if(bare)return <>{children}</>;const visibleNav=nav.filter(x=>canSee(x,profile?.role));
