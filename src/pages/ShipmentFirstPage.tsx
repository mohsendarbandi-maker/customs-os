...
const empty:Form={ownerId:'',shippingLine:'',billOfLading:'',year:String(new Date().getFullYear()),voyage:'',vessel:'',imo:'',flag:'',originPort:'',destinationPort:'',count:'',unit:'رول',net:'',gross:'',status:'draft',location:''};
...
export const ShipmentFirstPage:React.FC=()=>{
 ...
 return <main dir="rtl" className="min-h-screen p-4 md:p-6" style={{background:'var(--bg)',color:'var(--text)'}}><div className="max-w-[1500px] mx-auto space-y-4">
  ...
  <section className="rounded-2xl border app-border bg-[var(--surface)] p-5"><div className="grid lg:grid-cols-4 gap-3">
   ...
   <label className="text-xs app-muted">وضعیت کشتی<select value={form.status} onChange={e=>set('status',e.target.value)} className="w-full mt-1 rounded-xl border app-border bg-[var(--surface-2)] p-3">{statuses.map(s=><option key={s[0]} value={s[0]}>{s[1]}</option>)}</select></label><label className="text-xs app-muted">موقعیت فعلی کشتی<input value={form.location} onChange={e=>set('location',e.target.value)} className="w-full mt-1 rounded-xl border app-border bg-[var(--surface-2)] p-3"/></label>
