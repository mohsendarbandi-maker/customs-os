...
import { DeclarationOperationsChecklistPage } from './pages/DeclarationOperationsChecklistPage';
import { ShipmentFirstPage as ShipmentStartPage } from './pages/ShipmentFirstPage';
...
import { KnowledgeCenterV2Page } from './pages/KnowledgeCenterV2Page';
import { CustomsDocumentManagerPage } from './pages/CustomsDocumentManagerPage';
import { CaseAlertsPage } from './pages/CaseAlertsPage';
import { ControlCenterWithAlertsPage } from './pages/ControlCenterWithAlertsPage';
...
const operational: UserRole[] = ['owner', 'admin', 'broker', 'warehouse', 'client'];
const staff: UserRole[] = ['owner', 'admin', 'broker', 'accountant', 'warehouse'];
...
function LegacyDocumentsExtractRedirect() {
  const [p] = useSearchParams();
  const q = p.toString();
  return <Navigate to={`/documents${q ? `?${q}` : ''}`} replace />;
}

function LegacyExitRedirect() {
  const [p] = useSearchParams();
  const q = p.toString();
  return <Navigate to={`/operations?tab=exit${q ? `&${q}` : ''}`} replace />;
}

function Settings() {
  const { profile } = useAuth();
  const l = useLocation();
  const s = l.pathname.replace('/settings', '').replace(/^\//, '').split('/')[0];
  if (admins.has(s) && !['owner', 'admin'].includes(profile?.role || '')) {
    return <Navigate to="/settings" replace />;
  }
  return <SettingsEnterprisePage />;
}
            ...
            <AppShell>
              <Routes>
                ...
                <Route
                  path="/exit"
                  element={
                    <ProtectedRoute allowedRoles={['owner', 'admin', 'broker', 'warehouse']}>
                      <LegacyExitRedirect />
                    </ProtectedRoute>
                  }
                />
                ...
                <Route
                  ...
                  element={
                    <ProtectedRoute allowedRoles={operational}>
                      <LegacyDocumentsExtractRedirect />
                    </ProtectedRoute>
                  }
                />
                <Route
                ...
                />
                <Route
                  path="/declaration-print"
                  element={<Navigate to="/print-declaration" replace />}
                />
